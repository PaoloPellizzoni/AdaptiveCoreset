...
public class CoresetOblivious
{
    ...
    public void update(Point p)
    {
        ...
        last_points.add(p);
        r_t = minPairwiseDistance(last_points);
        M_t = Math.max(M_t, 2*first_point.distance(p));
        // end updating
        ...
        for(Integer gam : gams){
            if(gam < Math.floor(Math.log(r_t/2)/Math.log(1+beta))){
                RV.remove(gam);
                R.remove(gam);
                OV.remove(gam);
                O.remove(gam);
    ...
    double minPairwiseDistance(Iterable<Point> points){
        double ans = M_t*8+1;
        for(Point p1 : points){
            for(Point p2 : points){
                if(p1.distance(p2) > 0)
    ...
    TreeMap<Integer, TreeSet<Point>> O, OV;
    TreeMap<Integer, TreeMap<Point, Point>> R, RV;
